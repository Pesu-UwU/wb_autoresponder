...
class Autoresponder:
    ...
    def _compose_reply(self, obj) -> str: #  будет генериться с помощью api gpt
        if hasattr(obj, "mark"):
            prompt = (f"Ты автоответчик продавца товара на маркетплейсе Wildberries. Тебе нужно ответить на отзыв покупателя по следующим шаблонам. Ответы к положительным комментариям (оценка: больше или равна 4):"
                      ...
                      f"Текст отзыва: {obj.text}, оценка отзыва: {obj.mark}, имя клиента: {obj.user_name}."
                      f" Если у пользователя есть нормальное имя, то обратись к нему по имени. Избегай в своей речи предложений к дополнительным вопросам и пожеланиям. Без явной надобности от шаблонов не отходи")
